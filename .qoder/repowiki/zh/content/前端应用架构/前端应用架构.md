...
- [frontend/tourist-app/src/components/DigitalHuman/XunfeiAvatar.vue](file://frontend/tourist-app/src/components/DigitalHuman/XunfeiAvatar.vue)
...
T_xunfei["components/DigitalHuman/XunfeiAvatar.vue"]
...
- 数字人交互
  - DigitalHuman.vue：数字人容器组件，负责加载与切换不同渲染器（VRM、图片或讯飞），并协调动画与事件。
  - VrmAvatar.vue：VRM模型渲染器，基于WebGL/Three.js生态进行模型加载、材质与骨骼动画控制。
  - ImageAvatar.vue：图片型数字人渲染器，适用于轻量场景与低性能设备。
  - XunfeiAvatar.vue：基于科大讯飞 Web SDK 的实时数字人渲染器，通过 xrtc 协议拉流显示，支持文本驱动播报、自动播放恢复与优雅降级回 VRM。
...
### 数字人交互组件（类图）
数字人组件采用组合模式，将渲染器抽象为可插拔模块，便于在VRM、图片与讯飞渲染之间切换。
...
- 游客端依赖
  ...
  - 讯飞 Web SDK：前端 vendor/xunfei-sdk/ 目录下的 Avatar Web SDK v3.2.3-1002，运行时通过 `import()` 动态加载
