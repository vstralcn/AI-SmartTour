...
## 项目结构
数字人相关的前端代码位于 tourist-app 的 components 与 services 目录，后端服务在 digital_human 与 backend 中提供能力支撑。默认数字人模式为 XunfeiAvatar（基于科大讯飞 Web SDK），连接失败时自动降级为 VRM 或 ImageAvatar。整体关系如下：
...
- 讯飞数字人 XunfeiAvatar.vue：基于科大讯飞 Web SDK v3.2.3-1002 的实时数字人渲染器，通过 xrtc 协议接收云端音视频推流，文本驱动播报，支持自动播放策略处理与 VRM 降级。
...
> **Xunfei 模式**：默认优先尝试 Xunfei 连接，成功后音视频由云端直接推流，无需本地 TTS 合成和 WebGL 渲染。
...
职责与特性
- 数字人类型管理：支持讯飞、VRM 与图片三种渲染模式，默认优先尝试讯飞，失败后自动降级。
...
关键流程
- 初始化：读取配置（数字人类型、模型路径、音频策略等），按优先级依次尝试讯飞 → VRM → 图片。
...
LoadCfg --> TypeCheck{“数字人类型?”}
TypeCheck --> |讯飞| InitXunfei[“加载 Xunfei SDK/连接”]
TypeCheck --> |VRM| InitVRM[“加载 VRM 模型/纹理”]
TypeCheck --> |图片| InitIMG[“加载图片资源”]
InitXunfei --> Ready[“就绪”]
InitVRM --> Ready
InitIMG --> Ready
...
- 外部依赖
  - 讯飞 Web SDK（vendor/xunfei-sdk/）：通过 `import()` 动态加载，不参与 Vite 初始打包。
