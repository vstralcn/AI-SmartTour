...
- [XunfeiAvatar.vue](file://frontend/tourist-app/src/components/DigitalHuman/XunfeiAvatar.vue)
...
- 视图层：ChatView.vue
- 子组件：ChatPanel.vue（消息列表与输入区）、DigitalHuman.vue（数字人渲染与播放控制）、XunfeiAvatar.vue（讯飞数字人渲染）、VoiceInput.vue（语音采集与识别）
...
- DigitalHuman.vue：数字人容器组件，负责加载与切换不同渲染器（讯飞、VRM 或图片），并协调动画与事件。
- XunfeiAvatar.vue：基于讯飞 SDK 的实时数字人渲染，默认启用。
...
- 数字人播放：TTS 音频 → DigitalHuman.vue 播放控制 → 动画同步 → 状态回写至 chat.ts。讯飞模式下，音频与视频由讯飞云端直接合成推流，前端通过 xrtc 协议接收。
...
- 支持三种渲染模式：讯飞（默认）→ VRM → ImageAvatar。讯飞模式使用云端实时推流，不占用本地 GPU 资源。
