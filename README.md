...
| 能力 | 当前状态 | 说明 |
|---|---|---|
| 游客端与管理后台 | **基础产品闭环可用** | Vue 3 双端页面已接入真实后端数据，不再使用静态运营数据回退 |
| 文本流式问答 | **原型可用** | FastAPI + WebSocket；支持会话上下文与 Qwen 兼容 API |
| 景区知识检索 | **基础可用** | FAQ 与关键词检索返回来源、置信度和拒答；后台修改后立即热更新 |
| 知识库管理 | **基础可用** | PostgreSQL 持久化，支持景点/FAQ 编辑及 TXT、Markdown、PDF、DOCX 解析 |
| 个性化路线 | **基础可用** | 按游览时长、兴趣、同行人员和低强度约束筛选并展示规划依据 |
| 游客画像 | **基础可用** | 会话级画像保存兴趣、年龄阶段、同行人员、时长和行动偏好；账号级长期画像待实现 |
| 情感反馈 | **基础可用** | 规则分析驱动数字人表情，并将匿名情绪事件纳入后台统计 |
| 运营分析 | **基础可用** | 基于真实会话事件计算服务人次、响应时间、知识缺口、热点和路线偏好 |
| ASR / TTS | **兼容模式可用** | Chrome 浏览器实时中文 ASR 与 SpeechSynthesis 播报可用；Paraformer/CosyVoice 服务端推理待接入 |
| 2D 数字人 | **基础可用** | 后台角色配置与游客端联动，支持音色参数、情绪状态和播报驱动的基础口型动画 |
| MuseTalk 数字人 | **接口预留** | 已提供独立服务和 GPU Profile，真实视频口型推理链路尚未接通 |
| Agent 编排 | **基础可用** | 单编排器可识别知识、路线和反馈意图，调用白名单工具并返回执行轨迹 |
| 数据持久化 | **基础可用** | Docker 使用 PostgreSQL；本地开发使用 SQLite，保存知识、角色、会话与匿名交互事件 |
| 离线评测 | **重点规划** | 已具备可测试核心链路，标准问题集和自动化指标仍待补充 |
...
当前采用“FAQ 关键词匹配 + 轻量关键词排序 + 引用与置信度”的可运行检索链路；当证据不足时不编造答案，而是提示补充知识或转人工。管理后台支持文档上传解析、景点与 FAQ 编辑、即时索引和问答测试；BGE-M3、向量库与 Reranker 保留为下一阶段升级路径。
...
当前先落地了**轻量单编排器状态机**，复用现有 Python、FastAPI 和 Qwen 技术路线，可观察工具执行状态并在无大模型密钥时降级运行。后续流程复杂度提高后再迁移到 LangGraph；不采用复杂多 Agent 自由协作，降低开发和演示风险。
...
| 多模态交互 | 文本输入、浏览器中文 ASR、TTS 播报和 2D 口型状态联动 |
...
| 功能 | 竞争力设计 |
|---|---|
| 语音与口型同步 | 当前由浏览器 TTS 驱动 2D 口型状态；MuseTalk 视频口型为可选 GPU 升级 |
...
| 功能 | 竞争力设计 |
|---|---|
| 知识库治理 | 景点/FAQ 新增编辑、文档解析、来源、即时索引和问答测试 |
...
| 层级 | 保留方案 | 30 天内的升级重点 |
|---|---|---|
| 前端 | Vue 3、TypeScript、Vite、Pinia、ECharts | 已接入角色联动、浏览器语音、画像约束和真实运营看板 |
| API | FastAPI、WebSocket | 增加统一事件模型、流式音频协议、错误降级和链路追踪 |
| 大模型 | Qwen / OpenAI 兼容 API | 结构化输出、工具调用、低温度事实问答 |
| Agent | 轻量单编排器、白名单工具 | 流程复杂后迁移 LangGraph，不做高风险多 Agent 集群 |
| RAG | FAQ、关键词检索、结构化引用和阈值拒答 | 下一阶段接入 BGE-M3、向量库与 BGE Reranker |
| 语音 | 浏览器 ASR/TTS 降级，Paraformer、CosyVoice 适配器 | 下一阶段接入可私有化的服务端流式推理 |
| 数字人 | 2D 角色与口型状态、MuseTalk 接口 | 下一阶段接通 GPU 推理服务和音视频同步 |
| 数据 | PostgreSQL、SQLite、Redis | 已保存知识、角色、会话和匿名事件；账号及评测版本待补充 |
...
无需配置大模型密钥即可运行：系统会使用本地景区知识库、受控 Agent 工具和可信降级回答。需要 Qwen 增强生成时：
...
| Agent 工具清单 | `http://localhost:8000/api/v1/agent/capabilities` |

所有服务均配置健康检查；游客端和管理后台会等待后端就绪后再启动。查看日志与停止服务：
...
如需开发下一阶段的 BGE-M3 / ChromaDB 向量检索，再安装可选依赖：
